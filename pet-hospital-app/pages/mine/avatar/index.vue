	...
	const getImageInfo = () => {
		...
		uni.getImageInfo({
			...
			success:function(res){
				...
				ctx.draw(true, () => {
					// 获取画布要裁剪的位置和宽度   均为百分比 * 画布中图片的宽度    保证了在微信小程序中裁剪的图片模糊  位置不对的问题 canvasT = (cutT.value / cropperH.value) * (imageH / pixelRatio)
					...
					uni.canvasToTempFilePath({
						...
						quality: 0.5,
						canvasId: 'myCanvas',
						// filename: 'avatar' + userStore.name + '.png',
						success: function(res) {
							uni.hideLoading()
							let data = {
								name: 'avatarfile',
							...
							uploadAvatar(data).then(response => {
								if (isCloud) {
									userStore.avatar = response.imgUrl ? response.imgUrl : profile
								} else {
									//否则
	...
	const dragMove = (e) => {
		...
		switch (dragType) {
			...
			case 'left':
				...
				if ((CUT_L - dragLength) > (cropperW.value - cutR.value)) dragLength = CUT_L - (cropperW.value - cutR
					.value)
				setData({
			...
			case 'top':
				...
				if ((CUT_T - dragLength) > (cropperH.value - cutB.value)) dragLength = CUT_T - (cropperH.value - cutB
					.value)
				setData({
